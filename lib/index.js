...
import { StyleSheet, FlatList, ScrollView, View, ViewPropTypes, VirtualizedList} from "react-native";
...
class Column extends React.PureComponent {
	...
	render() {
		return <View style={{ flex: 1, overflow: "hidden" }}>
			<FlatList
				listKey={`flatlist_listkey_${this.props.index}`}
...
export default class Masonry extends React.PureComponent {
	...
	constructor(props) {
		...
		this.itemQueue = [];
		this.virtualizedListRenderItem = this.virtualizedListRenderItem.bind(this)
	}
	...
	virtualizedListGetItem(data, index) {
		return data[0]
	}

	virtualizedListGetItemCount(data) {
		return data.length
	}
	...
	render() {
		return (
			<VirtualizedList
				...
				listKey={"virtual_list_listKey_0"}
				data={[{ key: "virtual_list_0" }]}
				getItem={this.virtualizedListGetItem}
				getItemCount={this.virtualizedListGetItemCount}
				renderItem={this.virtualizedListRenderItem}
		...
		// 		</View>
		// 		{this.props.footer}
		// 	</ScrollView>    // old method, use scroll view, cause warning 
		// }
